...
The data table is the product; everything in `apps/web` exists to document and ship it. The primitives in `@workspace/ui` are in-repo dependencies — they are _not_ shipped to consumers (the registry declares them as `registryDependencies` so consumers get them from upstream shadcn in their own style).
...
- **Never mention Claude, AI, or any agent/tool** anywhere in git-related content — commit messages, branch names, PR titles, PR bodies, tags, or release notes. No attribution, no "generated by", no co-author trailers. Write everything as the repo user.
