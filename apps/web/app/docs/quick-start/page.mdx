...
<Callout variant="tip" title="Memoize data and columns">
  Pass stable references for `data` and `columns` (e.g. via `React.useMemo` or
  module scope). Handing TanStack a new `columns` identity every render can
  reset state.
</Callout>

## Where to go next

- [Usage](/docs/usage) — the options-in / instance-out model and `table.tableInstance`.
