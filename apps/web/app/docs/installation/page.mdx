...
<Callout variant="note" title="Any shadcn style works — Radix UI or Base UI">
...
1. **Installs npm dependencies** — `@tanstack/react-table`, `@tanstack/react-virtual`, `@tanstack/match-sorter-utils`, `@dnd-kit/*`, `date-fns`, `papaparse`, `xlsx`, `lucide-react` (plus `@types/papaparse`). The headless component library (Radix UI or Base UI) comes with the primitives below, matching your flavor.
