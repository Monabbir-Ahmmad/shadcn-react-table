...
<Callout variant="tip" title="Set sensible bounds">
  Define `minSize` and `maxSize` so users cannot collapse or stretch a column
  past a usable width.
</Callout>
...
```tsx
table.tableInstance.autoSizeColumn("firstName")
table.tableInstance.autoSizeAllColumns()
```
